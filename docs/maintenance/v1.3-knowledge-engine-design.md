...
原始 profile 已经定位到超过最低效果阈值的物理层，但还没有机制语义观测时，知识检索不能
返回空结果，也不能把候选改动中才会出现的 `mechanism.*=present` 当成改动前机会。此时只允许
一张通用 `source_verified` routing card 进入 `candidates`，并标记
...
进入 ChangeSet 或 `PURSUE`。动作完成后，不论结果是 observed、inconclusive、unavailable
还是 failed，同一自动测量入口都不再出现。若仍没有机制方向，返回现有 `STOP`，不通过改名
或更换 request id 重跑。
...
6. 机制关闭或候选失败后，只在现有提交事务完成时派生下一轮上下文。
