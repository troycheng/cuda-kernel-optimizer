...
ChatGPT 结合执行路径、源码和相关知识提出不超过三个竞争性假设。V1.3 的 12 个机制族覆盖 CUDA kernel、CUTLASS/CuTe、Triton、PyTorch、Serving 和 NCCL 的常见性能问题；
Ampere 至 Blackwell 的架构能力按精确 SM 和当前本地身份过滤。知识层只接受已封存的结构化观测，不把历史收益数字当作当前收益。
...
- 本地知识引擎提供 12 个机制族，覆盖六个软件层；Ampere 至 Blackwell 的来源支持已通过离线路由与架构反事实测试。
