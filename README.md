...
分析范围包括 CUDA、CUTLASS、Triton、PyTorch、vLLM、TensorRT-LLM，也包括框架调度、CPU 与数据处理、传输、
通信、I/O、allocator 和运行时状态。优化目标始终以用户提供的完整 workload 为准，不预设瓶颈一定在 kernel 内。
...
- 结合当前 workload 的封存证据、源码和知识资料，提出最多三个可证伪方向，并优先执行最低成本的检查。
...
ChatGPT 结合执行路径、源码和相关知识提出不超过三个竞争性假设。V1.3 只从当前身份、性能
模型和已封存的结构化观测生成方向，不把历史收益数字当作当前收益；相同环境中的既往失败
可以阻止重复尝试。Controller 再检查证据绑定、机制去重和结果层级，选择成本最低且能区分
假设的检查。新证据会更新下一轮判断，直到方向获得支持或被排除。
...
- 本地知识引擎根据当前身份、性能模型和已封存观测，返回不超过三个可证伪方向。
- 历史案例只用于精确身份下的支持或排除，不把历史收益迁移到新 workload。
- 现有六个 RTX 5090 Triton 决策点用于知识包回归，不计入发布门禁；正式发布需要独立的冻结后案例。
