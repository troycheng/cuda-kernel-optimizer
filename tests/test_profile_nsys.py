...
def _collection_fixture(temporary: str) -> tuple[dict, dict, Path]:
    ...
    driver.write_text(
        "\n".join(
            [
                ...
                "  'subject_digests': [{'role': subject['role'], 'digest': subject['variant']['digest']}],",
                ...
                "    'runtime_provenance': {'kind': 'host', 'identity': 'fixture-host', 'lineage_complete': True, 'lineage': [], 'components': []},",
                "  },",
                "  'acquisition': request['acquisition'],",
                "  'evidence': {",
                "    'correctness': [{'role': subject['role'], 'result': {'status': 'passed', 'metrics': {'exact_match': 1.0}}}],",
                "    'measurements': [{'role': subject['role'], 'result': {'primary': {'name': 'latency_ms', 'unit': 'ms', 'samples': [1.0]}, 'constraints': []}}],",
                "  },",
                "}",
    ...
    runtime_environment = {
        ...
        "gpu_architectures": ["sm_fixture"], "driver_version": "fixture",
        "cuda_runtime_version": "fixture", "frameworks": {},
        "runtime_provenance": {"kind": "host", "identity": "fixture-host", "lineage_complete": True, "lineage": [], "components": [], "attribution_scope": "identified_runtime"},
    }
    target = {
        "record_type": "target", "format_version": "cuda-kernel-optimizer/target-v2",
    ...
    request = {
        ...
        "operation_timeout_seconds": 8.0, "command_timeout_seconds": 3.0,
        "resource_wait_timeout_seconds": 1.0, "cleanup_timeout_seconds": 1.0,
        "launch_deadline": time.time() + 6.0,
    }
