...
外部搜索和第三方 AI 质证是可选的研究手段。优化方向依赖复杂系统归因、存在多个合理解释、影响核心 runtime/kernel path 或实验代价较高时，可请求外部模型挑战问题定义、假设和证据遗漏；外部模型不批准候选，也不接管执行。发送前移除私有内容，并保留不同意见。外网不可用不妨碍本地优化；最终由本地精度与测量证据决定，ChatGPT 据此完成判断。
