...
尚未量化 production gap，尚未确认当前 Candidate 的真实替换范围，证据不适用于该执行形态，收益上限低于 minimum effect，或测量分辨率不足以区分预期效果时，不得进入实现或正式性能测试；下一步只能补最便宜的必要观测，或者停止当前方向。发现社区已有等价能力时，回到复用或最小适配，不继续独立实现。
